...
int EncodeInt(int i, byte *bytes) {
  IntBytes ib;
  ib.i = i;
...
/*
  Copies an encoding of a 16-bit short to a buf that must contain at least 2
  bytes.
*/
int EncodeShort(short s, byte *bytes) {
  ShortBytes sb;
  sb.s = s;
...
*/
int EncodeCString(char *str, byte *bytes, int max_len) {
  int len = strlen(str);
  if (len + 2 > max_len) {
    len = max_len - 2;
  }
...
int DecodeCString(byte *bytes, char *str, int max_len) {
